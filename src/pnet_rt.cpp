...
Pnet::Pnet() :prototxt("/home/zhou/12net.prototxt"),model("/home/zhou/12net.caffemodel"),
...
//    if (firstFlag)
        ...
        CHECK(cudaMemcpyAsync(buffers[inputIndex],image.data,BatchSize*INPUT_C*INPUT_H*INPUT_W*sizeof(float),cudaMemcpyHostToDevice,stream));
        context->enqueue(BatchSize,buffers,stream, nullptr);
        CHECK(cudaMemcpyAsync(this->score_->pdata,buffers[outputProb],BatchSize*OUT_PROB_SIZE*sizeof(float),cudaMemcpyDeviceToHost,stream));
        CHECK(cudaMemcpyAsync(this->location_->pdata,buffers[outputLocation],BatchSize*OUT_LOCATION_SIZE*sizeof(float),cudaMemcpyDeviceToHost,stream));
